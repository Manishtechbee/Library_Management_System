      ...
      <div className="max-w-7xl mx-auto px-6 text-center">
        ...
        >
          {[{ icon: <FaUserShield size={28} />, title: "Secure Auth", desc: "Role-based login for Admins, Librarians & Students" },
            ...
            <motion.div
              key={title}
              className="p-6 rounded-xl bg-[#f3f8fd] shadow-md hover:shadow-lg transition"
              variants={cardVariants}
              whileHover={{ scale: 1.05 }}
              transition={{ duration: 0.1 }}
            >
              <div className="text-[#1b365d] mb-4">{icon}</div>
              <h3 className="font-semibold text-lg text-[#1b365d] mb-2">{title}</h3>
