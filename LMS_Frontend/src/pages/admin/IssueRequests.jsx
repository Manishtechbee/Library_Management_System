...
export default function IssueRequestsPanel({darkMode}) {
  ...
  return (
    <>
    {darkMode?(<div className={`p-6 rounded-xl shadow space-y-4 ${darkMode ? "bg-gray-800 text-gray-100" : "bg-white text-gray-800"}`}>
  <h2 className={`text-xl font-semibold mb-4 ${darkMode ? "text-gray-100" : "text-gray-800"}`}>Book Issue Requests</h2>

  <div className="overflow-x-auto">
    <table className="min-w-full text-sm">
      <thead>
        <tr className={`${darkMode ? "bg-gray-700 text-gray-200" : "bg-gray-100 text-left"}`}>
      ...
      <tbody>
        {requests.length === 0 ? (
          <tr>
            <td colSpan="6" className={`p-2 text-center ${darkMode ? "text-gray-400" : "text-gray-500"}`}>No requests found.</td>
          </tr>
        ) : (
          requests.map(req => (
            <tr key={req.id} className={`border-t ${darkMode ? "border-gray-700" : "border-gray-200"}`}>
              ...
              <td className="p-2 flex gap-2">
                <button
                  ...
                  className={`px-2 py-1 rounded text-sm ${
                    req.status !== "pending"
                      ? `${darkMode ? "bg-gray-600 text-gray-300 opacity-60 cursor-not-allowed" : "bg-gray-400 text-black opacity-60 cursor-not-allowed"}`
